...
### Step 5: Deploy to VPS (Optional)

**⚠️ IMPORTANT:** After exporting authentication, you'll be asked if you want to deploy to VPS.

If you choose to deploy to VPS:

1. **Automated Deployment Script** (recommended):
   ```bash
   /tmp/deploy_youtube_auth.sh
   ```

   This script will:
   - Upload authentication files to VPS (root@76.13.219.143)
...
2. **Manual Deployment**:
   ```bash
   # Upload to VPS
   sshpass -p 'PASSWORD' scp /tmp/youtube-storage-state.json \
     root@76.13.219.143:/root/.youtube/storage_state.json

   sshpass -p 'PASSWORD' scp /tmp/youtube-cookies.txt \
     root@76.13.219.143:/root/.youtube/cookies.txt

   # Test on VPS
   ssh root@76.13.219.143
   yt-dlp --cookies /root/.youtube/cookies.txt --skip-download \
     "https://www.youtube.com/watch?v=dQw4w9WgXcQ"
   ```

**🎯 Decision Flow:**

After Step 4, the skill will ask:
```
...
- **Y (Yes)**: Run automated deployment script
- **n (No)**: Skip deployment, use locally only

**📊 Deployment Testing:**

If you deploy, the script will automatically test:

1. ✅ File upload verification
2. ✅ Authentication test (Playwright)
3. ✅ yt-dlp download test
...
生成的文件:
  - Playwright 格式: /tmp/youtube-storage-state.json
  - Netscape 格式:   /tmp/youtube-cookies.txt
...
2. 使用 Playwright:
   context = await browser.new_context(storage_state='/tmp/youtube-storage-state.json')

3. 部署到服务器:
   scp /tmp/youtube-storage-state.json root@76.13.219.143:/root/.youtube/

⚠️  是否部署到 VPS 以便自动下载？[Y/n]
```

**VPS Usage After Deployment:**
...
# Or use remote command
sshpass -p 'PASSWORD' ssh root@76.13.219.143 \
  "yt-dlp --cookies /root/.youtube/cookies.txt 'VIDEO_URL'"
```
