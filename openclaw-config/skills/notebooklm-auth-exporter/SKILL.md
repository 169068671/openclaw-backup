...
**⚠️ VPS Limitation:** Google detects VPS IPs from datacenters and may block direct access. For NotebookLM, we recommend using authentication locally or with a proxy.
...
- **n (No)**: Skip deployment, use locally only (recommended)
...
3. ⚠️ VPS accessibility check

**Expected Output (with VPS limitation):**
```
========================================
  NotebookLM 认证导出工具
========================================

步骤 3: 测试 NotebookLM 认证
✅ NotebookLM 认证成功！
页面标题: NotebookLM
...
生成的文件:
  - Playwright 格式: /tmp/storage_state.json
...
⚠️  是否部署到 VPS？[Y/n]
注意: VPS 可能受 Google IP 限制，推荐本地使用。
```

**VPS Usage After Deployment:**

**Option 1: With Proxy** (if available):
```python
from playwright.async_api import async_playwright
import asyncio
...
async def use_notebooklm_on_vps():
    async with async_playwright() as p:
        ...
        # Configure proxy to bypass IP restrictions
        context = await browser.new_context(
            storage_state="/root/.notebooklm/storage_state.json",
            proxy={
...
**Option 2: Local Usage** (recommended, no VPS):
```python
from playwright.async_api import async_playwright
import asyncio

async def use_notebooklm_locally():
    async with async_playwright() as p:
        browser = await p.chromium.launch(headless=True)
...
asyncio.run(use_notebooklm_locally())
```

**Option 3: Use notebooklm CLI** (on VPS):
```bash
# On VPS
notebooklm login
