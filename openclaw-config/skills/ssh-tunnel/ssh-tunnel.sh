...
start_tunnel() {
    ...
    # 启动隧道
    ssh -i "$SSH_KEY" -N -D ${LOCAL_PORT} -f ${VPS_USER}@${VPS_HOST}
    local start_result=$?
...
show_status() {
    ...
    if [ -n "$pid" ]; then
        ...
        if ss -tlnp | grep -q "127.0.0.1:${LOCAL_PORT}"; then
            ipv4_status="✅"
        fi

        if ss -tlnp | grep -q "\[::1\]:${LOCAL_PORT}"; then
            ipv6_status="✅"
        fi
