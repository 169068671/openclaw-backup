# YouTube Access Skill - Quick Start
...
### yt-cookies-manager.py

```
用法: yt-cookies-manager.py <input.json> [OPTIONS]

选项:
  --playwright FILE    转换为 Playwright 格式
  --netscape FILE      转换为 Netscape 格式
  --validate           验证 cookies 的完整性
  --info               显示 cookies 的详细信息
  -h, --help           显示帮助信息
```
...
```
用法: yt-validate.sh [OPTIONS]

选项:
  -c, --cookies FILE   Cookies 文件路径（默认: /tmp/youtube_cookies.txt）
  -p, --proxy PROXY    代理地址（默认: socks5h://127.0.0.1:1080）
  -v, --verbose        详细输出
  -h, --help           显示帮助信息
```
...
```
用法: yt-download.sh [OPTIONS] <VIDEO_URL>

选项:
  -c, --cookies FILE   Cookies 文件路径（默认: /tmp/youtube_cookies.txt）
  -p, --proxy PROXY    代理地址（默认: socks5h://127.0.0.1:1080）
  -o, --output DIR     输出目录（默认: /tmp）
  -f, --format FORMAT  视频格式（默认: 最佳质量）
  -s, --subtitles      下载字幕
  -v, --verbose        详细输出
  -h, --help           显示帮助信息
```
