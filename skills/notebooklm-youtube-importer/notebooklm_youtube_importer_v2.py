#!/usr/bin/env python3
"""
NotebookLM YouTube 播放列表导入器（增强版）
...
def extract_videos_from_browser(youtube_url: str) -> list:
    """使用 agent-browser 提取 YouTube 播放列表中的所有视频链接"""
    
    print(f"使用 agent-browser 提取播放列表...\n")
...
async def import_videos_with_browser(youtube_url: str, notebook_name: str):
    ...
    # 提取视频链接
    video_urls = extract_videos_from_browser(youtube_url)
    ...
    # 创建笔记本并导入
    return await import_video_list(video_urls, notebook_name)


async def import_video_list(video_urls: list, notebook_name: str):
    """导入视频列表到笔记本"""
    
    print(f"导入 {len(video_urls)} 个视频到笔记本: {notebook_name}\n")
    ...
    async with await NotebookLMClient.from_storage() as client:
        ...
        print(f"   您可以稍后使用以下命令检查状态：")
        print(f"   notebooklm use {notebook_id}")
        print(f"   notebooklm source list")
